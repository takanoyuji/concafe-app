...
/**
 * 予約フォーム本体。**ログイン済みの会員だけ**が開く（page.tsx が入口で弾く）。
 * 通知先のメールアドレスは会員のものを使うので、フォームでは受け取らない。
 */
export default function ReserveForm({ email, defaultName }: { email: string; defaultName: string }) {
  const [stores, setStores] = useState<Store[]>([]);
  const [form, setForm] = useState({ ...EMPTY, customerName: defaultName });
        ...
        {done ? (
          <div className="glass p-6 mt-6 space-y-4">
            ...
            <p className="text-white/60 text-sm">
              受付のメールを <span className="text-white/80">{email}</span> にお送りしました。
          ...
          <>
            ...
            <form onSubmit={submit} className="glass p-6 space-y-5">
              ...
              {/* 通知先は会員のメールアドレス。ここでは変えられない（変えるならプロフィールから） */}
              <Field label="ご連絡先メールアドレス">
                <p className="text-white/80 text-sm py-2">{email}</p>
                <p className="text-white/50 text-xs">
                  受付・確定のご連絡をこのアドレスにお送りします
                </p>
              </Field>
