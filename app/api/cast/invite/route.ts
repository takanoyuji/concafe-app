...
/** GET /api/cast/invite?token= — 招待の有効性と表示名・ロール（パスワード設定画面の表示用） */
export async function GET(req: Request) {
  const token = new URL(req.url).searchParams.get("token") ?? "";
  const invite = await findValidInvite(token);
  if (!invite) return NextResponse.json({ error: INVALID }, { status: 400 });
  const role = invite.role === "MANAGER" ? "MANAGER" : "CAST";
  return NextResponse.json({ castName: invite.cast?.name ?? "", email: invite.email, role, roleLabel: ROLE_LABEL[role] });
}

/**
 * POST /api/cast/invite  { token, password }
 * パスワードを設定して User を作る。招待のロール（CAST / MANAGER）を付け、castId があれば Cast.userId に結ぶ。
 * そのままログイン状態にする
 */
export async function POST(req: Request) {
  ...
  const role: "CAST" | "MANAGER" = invite.role === "MANAGER" ? "MANAGER" : "CAST";
  ...
  const user = await prisma.$transaction(async tx => {
    ...
    const u = await tx.user.create({
      data: { email: invite.email, passwordHash, role, emailVerified: true, name: invite.cast?.name ?? null },
    });
    if (invite.castId) await tx.cast.update({ where: { id: invite.castId }, data: { userId: u.id } });
    return u;
  }).catch(() => null);
  if (!user) return NextResponse.json({ error: INVALID }, { status: 400 });

  const payload: SessionPayload = { userId: user.id, role, emailVerified: true, mustChangePassword: false };
