...
const FROM = process.env.MAIL_FROM_VLIVER ?? process.env.SMTP_FROM ?? "noreply@vliverlab.com";
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  await transport.sendMail({
    from: FROM,
    to: email,
    subject: "【VLiverLab】メールアドレスの確認",
    html: `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto">
        <h2>VLiverLab メール認証</h2>
...
export async function sendPasswordResetEmail(email: string, token: string): Promise<{ resetLink?: string }> {
  ...
  await transport.sendMail({
    from: FROM,
    to: email,
    subject: "【VLiverLab】パスワードリセット",
    html: `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto">
        <h2>VLiverLab パスワードリセット</h2>
