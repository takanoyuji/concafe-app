    ...
    <header className="fixed top-0 left-0 right-0 z-40 glass-dark">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1" aria-label="メインナビゲーション">
          ...
          {/* 予約は一律この導線に寄せる運用なので、他のナビより目立たせる */}
          <Link
            href="/reserve"
            className="flex flex-col items-center gap-0.5 px-3 py-2 text-star-400 hover:text-star-300 transition-colors text-xs font-bold"
          >
            <TicketIcon />
            <span>RESERVE</span>
          </Link>
        ...
        <div className="md:hidden glass-dark border-t border-white/10 py-4">
          <div className="grid grid-cols-3 gap-2 px-4">
            ...
            <Link
              href="/reserve"
              onClick={() => setOpen(false)}
              className="flex flex-col items-center gap-1 py-3 text-star-400 hover:text-star-300 transition-colors text-xs font-bold"
            >
              <TicketIcon />
              <span>RESERVE</span>
            </Link>
